...
INNER JOIN tallas on productos.Talla = tallas.TallaID WHERE escuelas.EscuelaID='".$_SESSION["escuela"]."' AND productos.Nombre='Playera'";
...
?>
  <form action="" method="POST">
    <section class="container" style="margin-top: 150px;">
        <div class="container">
            <div class="row">
                <div class="col-sm-12 text-center">
                    <h1 style="font-family: Arial,sans-serif;"><?php foreach ($tabla as $registro){ if($registro->ProductoID==$_SESSION["playera"]) {echo"$registro->Tipo";echo " "; echo "$registro->Escuela"; }} ?></h1>
                 </div><br><br><br>
                <div class="col-sm-6">
                   <img style=" box-shadow: 2px 2px 5px #999; border: 2px solid black; width:450px;height:450px;" src="../<?php foreach ($tabla as $registro){ if($registro->ProductoID==$_SESSION["playera"]) {echo"$registro->Imagen";}} ?>" alt="">
                </div>
                <div class="col-sm-6">
                    <label style="font-family: Arial,sans-serif;">Precio: <label style="color: red;" for="">$<?php foreach ($tabla as $registro){ if($registro->ProductoID==$_SESSION["playera"]) {echo"$registro->Precio";}} ?></label></label>  <br>
                    <label style="font-family: Arial,sans-serif;">Existencia: <?php foreach ($tabla as $registro){ if($registro->ProductoID==$_SESSION["playera"]) {echo"$registro->Existencia";}} ?></label>  <br>
            ...
            productos.ProductoID='".$_SESSION["playera"]."'";
            $tabla2 = $CONE->seleccionar($consulta);
            foreach($tabla2 as $registro)
            {
                if($registro->ProductoID==$_SESSION["playera"])
                {
        ...
        <?php
            if(isset($_POST["talla"])){$talla=$_POST["talla"]; $_SESSION["playera"]=$talla; echo "<meta http-equiv='refresh' content='0'>";
        }
        ?>
        <br>
        <div class="col-sm-6">
        <label class="control-label">Cantidad:</label>
        <input type="number" value="1" name="cantidad" min="1" max="<?php foreach ($tabla as $registro){ if($registro->ProductoID==$_SESSION["playera"]) {echo"$registro->Existencia";}} ?>">
        </div>
        <br>
        <div class="col-sm-6">
                    <button class="btn btn-primary btn-lg" type="submit" formaction="../scripts/agregarCarritoPlayera.php">Agregar al Carrito</button>
                </div>
                </form>
                </div>
